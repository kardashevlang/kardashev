...
**STATUS: ✅ SHIPPED (v0.90.0) — CLOSES THE v81–v90 ARC. Scope corrected by ground-truth (2 of the 3 named parts already half-met).** A grounded survey ran the compiler and corrected two survey errors: **(a) "mutable slices" don't exist in ANY backend** — there is no `slice_set`/`slice_get_mut`; mutation-through-slice is rejected even in LLVM (`E0384`). So the honest, parity-preserving cut is **read-only slices in the C backend** (it refused *all* slices). **(b) vectorization is ALREADY complete** across JIT/AOT/`--emit-llvm` (all share `codegen()→finish()`; verified: 16 vector ops in `--emit-llvm bench/loop.kd`, 17 SIMD in the AOT binary) — the survey's "JIT-only" claim was false; so the optimization "pass" is **verify-and-lock**, not a fix. **SHIPPED:** **(1)** read-only C-backend slices — `&[T]` → `struct kdslice { int64_t* ptr; int64_t len; }` (mirrors LLVM `{i8*,i64}`), `slice_len`/`slice_get`/`slice_get_ref` bounds-checked, `&v[a..b]` creation over a scalar Vec; scalar-element only (non-scalar `&[String]` cleanly refused — LLVM keeps it). **(2)** a permanent vectorization **regression gate** (IR-grep) locking the v51 TTI fix so a future PassBuilder refactor can't silently drop auto-vectorization again. **Gate:** `smoke_test_v90_close.sh` — slice read/subrange/get_ref each **JIT == AOT == C backend**, non-scalar-slice C-refusal, and the vector-op IR assertion. Full local smoke sweep + 6 unit suites green; CI both platforms. **Deferred (honest, no stubs) → a documented v91 line:** a user-replaceable **`GlobalAlloc`** allocator (L/XL — ~63 hardcoded malloc/realloc/free sites + free-glue routing, and not CI-safely-observable without fragile LD_PRELOAD; the survey's 63-site inventory is captured for v91); genuine **slice mutation** (`slice_set`/`slice_get_mut` — needs typecheck + borrow-check + both backends, exists in none today); slice-from-fixed-array in the C backend; and the <5-LOC array-layout-helper trim (low value, refactor risk). The closing version ships the real, testable cuts and names the rest honestly rather than stubbing a half-allocator.
