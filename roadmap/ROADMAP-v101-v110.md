...
**STATUS: ✅ SHIPPED (v0.106.0) — LOCK-ONLY (no codegen change), the v95 pattern.** Closes ARC B. Live probing proved the premise is **already met in the default build**, so — exactly as the version's own theme anticipated and the v95 precedent dictates — v106 ships **no codegen change** (one would be a no-op stub or a regression risk) and instead ships a **permanent gate that locks the win**. Probed + independently re-verified on x86-64: (a) a self-tail-recursive `fn sum(n, acc)` has its recursive call **fully eliminated at -O2** (0 surviving `call @sum` — IndVars/SCEV solve it to closed form; at -O1 it's a `tailrecurse` loop), and `sum(1_000_000, 0)` **runs to completion** (exit 0, prints `500000500000`) with **no stack overflow** — it overflows only at explicit `-O0` (a deliberate opt-out); (b) a monotone array loop with a statically-provable bound has its bounds checks **fully elided** (`panic_oob`/`idx.panic` = 0) and **vectorizes** (37 vector ops, the v51 TTI win); (c) a `vec_get` monotone counted loop has its range/sign/panic checks **SCEV-eliminated** (0/0/0). **GATE:** `smoke_test_codegen_tco.sh` — deterministic, target-aware, zero wall-time: BLOCKING structural IR-greps (0 surviving `call @sum`; array loop 0 bounds checks; vec_get loop 0 range/sign/panic — all target-independent transforms, so enforced on both arches) + a **runtime no-overflow + correctness proof** (`sum(1e6,0)` exit ≠ 139 and == `500000500000`) + a correctness oracle on the loops; vectorization is x86-64-enforce / arm64-soft (the v90 lesson). Complements the v95 perf-lock + v90 vector-lock. (Verified in-session: all 6 gate checks pass; full smoke sweep; clean rebuild reports 0.106.0.) **DEFERRED, honestly (sized):** TCO at explicit **`-O0`** (the user opted out of optimization — guaranteeing it there means a TailCallElim FunctionPass in the O0 pipeline, changing the O0 contract; deliberate non-goal); a **`become`/`musttail` language guarantee** (a hard compile-error-if-not-a-tail-call, vs the optimizer's best-effort — needs a lexer keyword + `setTailCallKind(MustTail)` + legality check); **general/mutual** tail-call-elimination *guarantee* (mutual works incidentally via O2 inlining but isn't promised); the **`vec_get` null-data branch** that blocks by-value-Vec-loop vectorization (correctness-neutral + off the benchmark surface — no `bench/*.kd` uses a Vec hot loop, and the by-ref `vec_get_ref` path already vectorizes); LTO / cross-module inlining (XL).
...
**STATUS: ✅ SHIPPED (v0.107.0).** The self-hosted emitter `examples/selfhost/structgen.kd` now genuinely **parses and lowers** a generic two-variant enum and a value-producing `match`, self==host gated. Live-probe-grounded (research workflow `w9sa01eh6`); independently verified in-session — `tests/smoke_test_selfhost_enum_match.sh` is green (12 checks: byte-identity R0, just/nope paths, both-arms, arm-order independence, binder-in-expr, inferred let-bound enum, + two negatives), as are the 7 prior self-host gates and the full 266-test sweep. **What shipped:** (a) a fixed generic enum `Opt<T> { Just(T), Nope }` — recognized by name (`Opt`/`Just`/`Nope`/`match`) exactly as the `str_*`/`vec_*` builtins are; the declaration is genuinely parsed + consumed (`skip_enum_decls`); a param `o: Opt<i64>` resolves to **tag 500** = the tagged struct `{ i64, i64 }` (so an enum is passed by value, like a struct). (b) Constructors lower to an `insertvalue` chain — `Just(x)` → `{0, x}`, `Nope` → `{1, undef}` (mirroring `SLit`). (c) `match o { Just(b) => .., Nope => .. }` is one new `Expr` variant `Match(scrut, just-binder, just-body, nope-body)` lowered as `extractvalue` of tag+payload, the Just binder bound to the payload in a **child env** (`clone`+`hashmap_insert` — host-verified independent copy), both arm bodies lowered, then a `select` on `tag == 0`. The `select` form mirrors the existing If-as-value lowering, so **match/if expressions emit NO branches** (single basic block) and **compose, even nested** — and it stays robust against the `=>` two-token quirk (`=` kind 14 + `>` kind −1). The new `ECon`/`Match` arms were added to all 7 `Expr`-match sites (type_of/lower/mono_structs/mono_fns/collect_strs/expr_uses_rt/is_str_concat); **no new `Stmt` variant**, so the 6 `Stmt`-match sites are untouched. **R0:** a no-enum program is byte-identical (verified `diff`-clean vs the pre-v107 binary). **DEFERRED, honestly:** the recognizer is keyed to the fixed `Opt`-shape (arbitrary enum/variant names + >2 variants deferred — like v94's single-`T` generics start); single i64 payload only (multi-payload / non-i64 / struct payloads deferred); scrutinee must be an OWNED enum value (no `match &o`); value-producing arms only (side-effecting arms would need branch+phi, not `select`); `let x: Opt<i64> = ...` annotations are not used (structgen has never supported `let` type annotations — a separate pre-existing limitation; bindings infer the type). Where the original plan said "real CFG via v91 `br`/`phi`": the `select` form is observationally equivalent for value-only arms (the gate is exit-code differential), simpler, and composes — so v107 uses it and defers branch+phi to the day side-effecting arms are needed.

**(original plan as authored:)**
...
**STATUS: ✅ SHIPPED (v0.108.0) — closes ARC C.** The self-hosted emitter `examples/selfhost/structgen.kd` now genuinely **parses and lowers** a heap-allocated single `i64` — `Box::new(e)` + `*b` — to real self-contained IR, freed once at the function exit, **self == host gated AND AddressSanitizer-clean**. Live-probe-grounded (research workflow `w8pn39kbb`); independently re-verified in-session (the host accepts only `Box::new` — `box`/`box_new` are rejected; the host frees with a drop-flag, structgen frees unconditionally at its single exit, which ASan confirms leak/UAF/double-free-free). **What shipped:** (a) a **`::` token** added to the lexer (byte 58 twice → kind 29) — structgen had none (`:` is a single token); (b) **`Box::new(e)`** recognized by name in `parse_call_or_var` (like `Just`/`Nope`/`vec_new`), lowering to `call ptr @malloc(i64 8)` + `store i64 <e>`, value tag **600** (a bare `ptr`, resolved by the existing `>= 200` arm in `ty_llvm`); (c) a **prefix-`*` deref** in `parse_factor` (kind 11 at a factor start — positionally distinct from infix multiply, which `parse_term` only sees after a left operand), lowering to `load i64, ptr`; (d) **Drop**: a `let mut` Box is tracked in `cg.owned` (tag 600) and freed at the single fn exit (`load ptr` from its slot + `call void @free`) — sound because the subset has no early return (`check_fn` already rejects a return tag `>= 200`, so a Box can never escape); (e) the libc `malloc`/`free` declares are emitted for a Box-only program via a new `want_box` runtime-family flag, leaving every prior gate byte-identical. The new `BoxNew`/`Deref` `Expr` arms were added to all 8 `Expr`-match sites; **no new `Stmt` variant**. **Verification:** `tests/smoke_test_selfhost_box.sh` (10 checks: R0 no-`@malloc`/`@free` byte-identity, malloc(8)+store+load+free IR shape, 2-malloc/2-free balance, box-in-helper-fn, two negatives [`Box::new(bool)`, `*<i64>`], every positive **self == host == ASan**); 8 prior self-host gates green; full 267-test sweep green. **DEFERRED, honestly:** `Box<i64>` only (no Box of struct/String/bool/generic-`T` — mirrors `Opt<i64>`=500 / `Vec<i64>`=4); read-only deref (host has no deref-assign); no returning a Box and no Box-typed params (a Box stays a within-fn `let mut` local); no nested Box-of-Box; only the FINAL value of a reassigned `let mut` box is freed (drop-on-reassign needs liveness — same limit as owned String); a plain immutable `let p = Box::new(..)` lowers + typechecks but isn't freed (no slot) so the gate uses `let mut`.
...
**STATUS: ✅ SHIPPED (v0.109.0).** Opens ARC D. **Scope corrected by live probing** (research workflow `wg1nxd1fu`): `assert!`/`assert_eq!`/`assert_ne!` **already exist** (prelude `macro_rules!`, the v37 effect-free `test_*() -> i64` convention — a failed assert `return`s 1 so the runner reports FAILED without aborting). So v109 ships two *additive*, real capabilities without regressing that convention: **(A) the Rust-semantics PANIC-form asserts** `expect!`/`expect_eq!`/`expect_ne!`, usable **anywhere** (not just `test_*`), which on failure `panic(format!("assertion … failed\n  left: {:?}\n right: {:?}", l, r))` — aborting with **exit 101** and a Debug-formatted left/right message. Built entirely on extant machinery (the `panic(String)` runtime + parser-desugared `format!` + the `Eq` trait's `.eq()` + `{:?}` Debug), so they generalize to **any `T: Eq + Debug`** (i64, bool, String, Option, Result, …) — verified that `==` is i64/bool-only, hence `.eq()`. Cost is honest: the panic form forces the caller to declare `! { alloc, panic }`, which is exactly why the effect-free return-1 asserts stay the default for tests. **(B) `kard bench` / `kardc --bench`** — discovers `bench_*() -> i64` fns (mirroring `--test`/`isTestFn`), JIT-runs each, times it in the C++ host with `std::chrono::steady_clock` (sidestepping `monotonic_millis`'s `io` effect + ms-only floor), and prints `bench <name> ... <ms> ms (result=<r>)`; `--filter` narrows; a `kard bench` wrapper case + help line added. The bench fn returns a deterministic checksum so the **gate asserts the result/structure, never wall-time** (CI timing is nondeterministic — the v95/BENCHMARKS philosophy). **Verification:** `tests/smoke_test_assert_v109.sh` (5 checks: existing return-1 convention intact [regression guard], `expect_eq!` aborts 101 JIT + AOT with Debug values, passing form is a no-op, String operands via `.eq`+`{:?}`) and `tests/smoke_test_kard_bench.sh` (5 checks: discovery + result correctness `result=499500`/`75025`, count, no-bench→error, `--filter`, the `kard bench` wrapper); full 269-test sweep green. **DEFERRED, honestly:** the effect-free return-1 reporter stays i64-only (use `expect_*` or `assert!(a.eq(&b))` for non-i64 in tests); a panic-catching test runner (so the panic form could be the test default) — needs a catch boundary in `runTests`; advisory wall-time regression thresholds + statistical sampling (min/median/stddev — needs a sub-ms timer; `monotonic_millis` is ms-only); `--format=json` for bench; non-i64 bench return types (same ABI deferral as generic `thread_join<T>`).
...
**STATUS: ✅ SHIPPED (v0.110.0) — the FINAL version of the v101–v110 arc; closes ARC D and completes the arc.** Researched directly on the live binary (the research workflow hit a transient harness fault, so the probing + design were done in-session by hand). **(A) Bound-satisfaction diagnostics** (`compiler/src/typecheck.cpp`): when method resolution fails for a concrete type (a missing trait `impl`), the two error sites now emit a clear, actionable **E0277** — instead of the bare `no impl for type 'X' (method 'm')`, the message names the trait the method comes from (**the trait bound `X: Trait` is not satisfied**), suggests the exact fix (**add `impl Trait for X`**), and lists the types that DO provide the method (scanned from `traits_` for the trait + `methodImplLookup_` for the candidates). A direct missing-method call gets a **correct caret** on the call. **(B) LSP code actions** (`compiler/src/lsp_main.cpp`): the server advertises `codeActionProvider` and implements `textDocument/codeAction` — it re-runs the pipeline and, for each bound diagnostic carrying the `` add `impl …` `` hint, returns a **quick-fix `CodeAction`** whose `WorkspaceEdit` inserts an `impl` skeleton at the end of the file (parsing the impl signature straight out of the v110-A message — so A and B compose). **Verification:** `tests/smoke_test_bound_diag.sh` (4 checks: direct call names the bound + fix + correct caret + E0277; `#[derive(Eq)]` over a non-Eq field names `Inner: Eq`; the help lists candidate types; a satisfied bound still compiles + runs — no false positive) and `tests/smoke_test_lsp_codeaction.sh` (3 checks: capability advertised; the quick-fix offers `` Add `impl Show for Widget` `` with a `WorkspaceEdit` inserting the stub; a clean file yields `[]`). Full 271-test sweep green. **DEFERRED, honestly:** the bound check for a generic CALL site whose type-param is bound to a concrete type lacking the impl still surfaces at **codegen** (`impl method not declared`) rather than typecheck — a deeper monomorphization-time check; the `#[derive]` diagnostic's caret still points into the synthesized prelude region (the message names the type correctly — fixing the span needs derive-site provenance threading); the inserted `impl` stub is an empty block (auto-generating the trait's method signatures is future work).
