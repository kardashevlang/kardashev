...
kardashev is **pre-1.0**. Every numbered roadmap through **v110** has shipped (v0.1.0 – v0.110.0). The **v101–v110 "production-depth" arc** ([roadmap/ROADMAP-v101-v110.md](roadmap/ROADMAP-v101-v110.md)) is **complete across all four sub-arcs**: **ARC A — stdlib depth** (v101 element-generic iterators, v102 recursive container `Debug`, v103 quicksort + `binary_search`, v104 slice utilities); **ARC B — type/codegen depth** (v105 generic `Eq`/`Hash` for `Option`/`Result` + `#[derive(Eq,Hash)]` composite HashMap keys, v106 a codegen tail-call + bounds-elision lock proving `-O2` lowers self-tail-recursion to a loop and elides monotone bounds checks); **ARC C — self-hosting** (v107 self-hosted enum + `match` lowering a generic `Opt<T> { Just(T), Nope }` to a tagged-struct `extractvalue` + `select`, v108 self-hosted `Box<i64>` heap indirection `Box::new`/`*b` → `malloc`/`store`/`load` freed at fn exit — both self == host gated, v108 also AddressSanitizer-clean); **ARC D — DX** (v109 Rust-semantics `expect_*` panic asserts for any `T: Eq + Debug` + a `kard bench` harness, v110 trait-bound-satisfaction diagnostics naming the bound + the fix + candidate types, with an LSP `codeAction` quick-fix that inserts the missing `impl`). Each release is green on a cleared clean build (6 unit suites plus the full smoke / fuzz aggregate, **JIT and AOT**) on **Linux + macOS** CI. The per-version detail lives in **[CHANGELOG.md](CHANGELOG.md)**; the current release is **[v0.110.0](https://github.com/kardashevlang/kardashev/releases/latest)**.
