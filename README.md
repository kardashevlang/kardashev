...
> **Status: planned, not yet implemented.** The phases below are the *intended* v8 (Phases 45–50). As in v1–v7, each phase ships fully green (6 unit suites + the smoke-test aggregate, JIT **and** AOT) before the next builds on it. Statuses are a **planned marker (🎯)**, not the shipped checkmark.

v7 gave kardashev floats, generic `impl<T>` blocks, `#[derive]`, and a JSON-2.0 capstone — but that capstone had to drop to an order-preserving `Vec<Entry>` for objects because a generic `Clone`/`Eq` **trait impl** for `HashMap` still can't be written (a generic-impl body can't satisfy the HashMap-op `K: Hash + Eq` requirement from a *bounded type param*, and a pure trait method can't host an `alloc`-using impl). v8's theme is **generics, finished**: make bounded type params first-class inside container ops, ship the container trait impls (restoring HashMap-backed JSON), round out `#[derive]`, and add the `Ord`/`sort` the stdlib has been missing — proving it with a JSON 3.0 capstone whose objects are real `HashMap`s again, with canonical sorted-key output.

| Phase | Goal | Status |
|-------|------|--------|
| 45 | **Bounded type params inside container ops** | 🎯 *Planned.* A bounded generic param `K: Hash + Eq` must satisfy the `HashMap<K, V>` / `HashSet<K>` key requirement (and an effect-row relaxation so a trait method may host an `alloc`-using impl): today `keyIsHashable` only accepts concrete i64/String/user-with-impl, so a generic-impl body over a map fails. **Accept:** a hand-written `fn count_keys<K: Hash + Eq, V>(m: &HashMap<K,V>) -> i64` compiles + runs for `HashMap<String,i64>`, and a generic-impl method whose body builds/queries a `HashMap<K,V>` typechecks — JIT + AOT. |
| 46 | **Generic `Clone`/`Eq` trait impls for `HashMap`/`Box`** | 🎯 *Planned.* With 45, ship prelude `impl<K: Hash+Eq, V: Clone> Clone for HashMap<K,V>` + `impl<K: Hash+Eq, V: Eq> Eq for HashMap<K,V>` (order-independent) and `Clone`/`Eq` for `Box<T>`, so `.clone()`/`.eq()` dispatch through entries rather than the structural intrinsic. **Accept:** a `HashMap<String, Vec<i64>>` deep-clones via `.clone()` and two such maps compare via `.eq()` order-independently, constant-memory, JIT + AOT. |
| 47 | **`Ord` trait + generic `sort`** | 🎯 *Planned.* An `Ord` trait (`cmp(&self, &Self) -> i64`, −1/0/1) with built-in impls for i64/f64/String, and a generic `fn sort<T: Ord>(v: &mut Vec<T>)` (in-place, the first real stdlib algorithm over a user bound). **Accept:** `sort` orders a `Vec<i64>`, a `Vec<String>`, and a `Vec<T>` of a user `Ord` type — JIT + AOT. |
| 48 | **`#[derive(Hash, Ord, Default)]`** | 🎯 *Planned.* Extend the derive machinery to `Hash` (combine field hashes), `Ord` (lexicographic field/variant compare), and `Default` (field-wise defaults), reusing 42's generate-and-reparse approach. **Accept:** a `#[derive(Hash, Eq)] struct K { ... }` is usable as a HashMap key; a `#[derive(Ord)]` type sorts; a `#[derive(Default)]` type constructs via `default()` — JIT + AOT. |
| 49 | **`dyn Trait<T>` — generic trait objects** | 🎯 *Planned.* Close the Phase-21 gap: a `dyn Trait<Arg>` object (vtable + data fat pointer) for a *parameterized* trait, so `&dyn Display`-style dynamic dispatch works for generic traits. **Accept:** a `Vec<Box<dyn Display>>`-style heterogeneous collection is iterated and each element formatted via dynamic dispatch — JIT + AOT. *(If `Box<dyn>` lifetime/ownership proves unsound this phase, `&dyn` is the floor.)* |
| 50 | **Capstone: JSON 3.0 — HashMap objects + full derive + canonical output** | 🎯 *Planned.* Restore `JObj(HashMap<String, Json>)` (the v6 north-star shape) now that 45–46 make it `#[derive(Clone, Eq)]`-able; serialize with **sorted keys** (via 47's `sort`) for canonical, byte-stable output; round-trip by derived `Eq`. **Accept:** parse `{"b":2,"a":[1,2.5],"c":"x\ny"}`, re-serialize canonically (keys sorted), round-trip by derived `Eq`, constant peak-RSS across a parse-and-drop loop, JIT + AOT, wired into `make -f Makefile.local test`. **The north star — HashMap JSON, fully derived.** |

**Landing order — 45 → 46 → 47 → 48 → 49 → 50.** 45 makes bounded params first-class in container ops (the blocker for everything map-generic). 46 uses it to ship the container trait impls. 47 adds `Ord`/`sort` (needed for canonical output). 48 extends `#[derive]` to Hash/Ord/Default. 49 adds dynamic dispatch for generic traits. 50 integrates into a HashMap-backed, fully-derived, canonically-serialized JSON 3.0. Each phase ships fully green before the next, as v1–v7 did.
