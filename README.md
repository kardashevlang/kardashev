...
kardashev is **pre-1.0**. Every numbered roadmap through **v100** has shipped (v0.1.0 – v0.100.0), and the **v101–v110 "production-depth" arc** ([roadmap/ROADMAP-v101-v110.md](roadmap/ROADMAP-v101-v110.md)) is now underway — v101 **element-generic iterator adaptors**, v102 **recursive container `Debug`** (`println!("{:?}", vec)` + `#[derive(Debug)]` over container fields). Each release is green on a cleared clean build (6 unit suites plus the full smoke / fuzz aggregate, **JIT and AOT**) on **Linux + macOS** CI. The per-version detail lives in **[CHANGELOG.md](CHANGELOG.md)**; the current release is **[v0.102.0](https://github.com/kardashevlang/kardashev/releases/latest)**.
