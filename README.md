...
kardashev is **pre-1.0**. Every numbered roadmap through **v100** has shipped (v0.1.0 – v0.100.0), and the **v101–v110 "production-depth" arc** ([roadmap/ROADMAP-v101-v110.md](roadmap/ROADMAP-v101-v110.md)) is now underway — v101 **element-generic iterators**, v102 **recursive container `Debug`**, v103 **quicksort + `binary_search`**, v104 **slice utilities** (`slice_to_vec`/`slice_iter`/`chunks`/`windows`) — closing ARC A; v105 **generic `Eq`/`Hash` for `Option`/`Result`** (+ `#[derive(Eq,Hash)]` over Option fields → composite HashMap keys), v106 **codegen tail-call + bounds-elision lock** (a permanent gate proving `-O2` already lowers self-tail-recursion to a loop and elides monotone bounds checks — closing ARC B), v107 **self-hosted enum + `match`** (the self-hosted compiler now lowers a generic `Opt<T> { Just(T), Nope }` + value-producing `match` to a tagged-struct `extractvalue` + `select`, self == host gated — opening ARC C), v108 **self-hosted `Box<i64>` heap indirection** (`Box::new(e)` → `malloc`+`store`, `*b` → `load`, freed once at fn exit; self == host **and** AddressSanitizer-clean — closing ARC C), v109 **`expect_*` panic asserts + `kard bench`** (Rust-semantics `expect!`/`expect_eq!`/`expect_ne!` that abort with exit 101 + a `{:?}` left/right message for any `T: Eq + Debug`, plus a `kard bench` harness that times `bench_*` fns and asserts their result — opening ARC D). Each release is green on a cleared clean build (6 unit suites plus the full smoke / fuzz aggregate, **JIT and AOT**) on **Linux + macOS** CI. The per-version detail lives in **[CHANGELOG.md](CHANGELOG.md)**; the current release is **[v0.109.0](https://github.com/kardashevlang/kardashev/releases/latest)**.
