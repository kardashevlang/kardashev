...
> **Status: planned, not yet implemented.** The phases below are the *intended* v6 (Phases 33–38); none has shipped. As in v1–v5, each phase will ship fully green (6 unit suites + the smoke-test aggregate, JIT **and** AOT, ubuntu + macOS via Bazel CI / the `Makefile.local` clang shim locally) before the next builds on it. Statuses are a **planned marker (🎯)**, not the shipped checkmark.
...
| Phase | Goal | Status |
|-------|------|--------|
| 33 | **Container interior drop + rehash reclaim + the missing `%`/`&&` operators** | 🎯 *Planned.* Generalize the proven `emitDropVecElements` template (codegen.cpp:6124) into the HashMap/HashSet `emitDropGlue` arm (6129–6136) so a map/set with droppable K/V drops every live key+value over occupied buckets before freeing the array; free the **old** bucket buffer on rehash-grow (2069–2071, the `we leak instead` site). Piggyback `%` modulo (lexer `Percent` → `*`-tier → LLVM `SRem`) and `&&` short-circuit (lexer two-char `AmpAmp` before single `&` → branch IR). **Accept:** a `HashMap<String, Vec<i64>>` built/dropped in a 2 M-iteration loop holds flat peak-RSS under the existing `smoke_test_dropleaks` probe; `a % b` and `x && y` green JIT + AOT. *(Only `%`/`&&` — **not** `\|\|`, already the zero-param-closure token.)* |
| 34 | **Read-without-move: borrow into containers + `match &x` / `match *r`** | 🎯 *Planned.* `vec_get_ref(&v,i) -> &T` / `hashmap_get_ref(&m,k) -> &V` return a **borrow**, not the current shallow copy; matching through a `&Enum` binds payload sub-patterns **as references** (replacing the hard error at typecheck.cpp:5598–5602/5625–5627 with a one-layer auto-deref mirroring the existing receiver auto-deref at 3479–3483). **Accept:** a `Vec<String>` and a `Vec` of a heap-owning enum are read/`match &`'d element-by-element, the originals stay valid and drop exactly once, constant-memory JIT + AOT. *The keystone — closes the root cause of the v5 JSON subset.* |
| 35 | **Sound recursive heap enums + a `Clone` trait** | 🎯 *Planned.* Codegen + `emitDropGlue` + borrow-check + the monomorphization worklist agree on a recursive enum whose payload is `Box<Self>`/`Vec<Self>`/`HashMap<K,Self>` (Box-drop already recurses-then-frees, 6072–6085 — the sound spine), with a codegen visited/depth guard; a prelude `Clone` trait with built-in deep-clone for Vec/String/HashMap/Box and **hand-written** impls for user types (no `#[derive]` — explicitly out of scope). **Accept:** a `Tree { Node(Vec<Tree>), Leaf(i64) }` 1000 nodes deep is `match &`-traversed, cloned, and dropped in a constant-memory loop with no leak/double-free, JIT + AOT. |
| 36 | **Enum-typed struct fields + non-Copy aggregate elements + `Result` errors** | 🎯 *Planned.* An enum/owning struct field stored at full fidelity (retiring the i64-code workaround calc/kdlex use); relax `isCopyAggregateElem` (typecheck.cpp:5244–5265) so a tuple may hold non-Copy elements (`(Json, i64)`, `(String, i64)`) **with** borrow-check move/partial-move tracking through element copies; tuple `match` patterns; a parser returning `Result<Json, ParseError>` via the shipped `?`. **Accept:** a struct with a recursive-enum field, a fn returning `(String, i64)`, and a `match` on a tuple / `Result` all build/read/drop exactly once, JIT + AOT. *(Array non-Copy elements scoped to tuples+fields if array move-tracking isn't provably sound; full `&mut` auto-reborrow stays deferred.)* |
| 37 | **`Display` / formatting + de-`i64`'d generic iteration & slices** | 🎯 *Planned.* A prelude `Display`/`ToString` trait (`to_string(&self) -> String`) with built-in impls + a `str_concat`/`str_push` builder; `checkFor` binds the loop variable to the iterator's **real** `next()` payload type instead of `elemTy = makeInt()` (typecheck.cpp:5098–5100); `&[T]` slices generic per element `T` (today i64-only, typecheck.cpp:444 / codegen.cpp:1748/7029), kept **non-owning** by borrow-check. **Accept:** `print_all<T: Display>(v: &Vec<T>)` iterates a `Vec<String>` and a `Vec<Json>` yielding real `T` and formats each; a `&[String]` slice read — JIT + AOT. *(`Display` via static dispatch only; `dyn Display` hits the open Phase-21 generic-`dyn Trait<T>` gap → deferred.)* |
| 38 | **Capstone: FULL nested-JSON (parse + serialize, leak-free) + doc-truth fixup** | 🎯 *Planned.* Promote `examples/json/` to the full recursive `enum Json { … JArr(Vec<Json>), JObj(HashMap<String, Json>) }`: parse nested input, traverse by reference (P34), re-serialize via `Display` (P37), all in kardashev. **Accept:** parse `{"a":[1,{"b":true},"x"],"n":null}`, round-trip-equality assert, constant peak-RSS across a parse-and-drop loop, JIT + AOT, wired into `make -f Makefile.local test`. Also correct the **stale** closure-env `not freed today` comments at codegen.cpp:8519–8520/8598–8599 (Phase 29 closed that leak at 6047) so `doc-lint` stays honest. **The north star.** |

**Intended landing order — 33 → 34 → 35 → 36 → 37 → 38 — along a strict dependency arc.** 33 establishes the interior-drop + constant-memory leak-probe discipline on the proven Vec template (lowest risk first) and folds in the two cost-free firsthand operators. 34 is the **keystone**: read/`match`-by-reference is the root-cause fix for the v5 subset, so it precedes everything that touches recursive data. 35 makes the recursive value itself a sound droppable/clonable heap citizen (needs 34's by-ref traversal to reduce clone frequency). 36 gives the AST its node *shape* (enum-typed fields, non-Copy tuples) — tractable only once 34–35 settled Drop/borrow semantics for owning elements. 37 supplies the `Display` serializer backbone and de-`i64`'s iteration so the tree can be walked over real `T`. 38 integrates all five into the full-nested-JSON capstone — the integration proof that surfaces any latent leak/UAF under its constant-memory gate. Each phase ships fully green before the next builds on it, exactly as v1–v5 did.
