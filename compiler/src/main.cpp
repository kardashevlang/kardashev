...
std::string applyPrelude(const std::string& userSrc) {
    ...
    if (userSrc.find("trait Ord") == std::string::npos) {
        prelude +=
            ...
            "    res\n"
            "} }\n"
            // v103: generic in-place sort — quicksort, median-of-three pivot +
            // insertion-sort cutoff (<=12). NOT stable (the old insertion sort
            // was) — equal elements may reorder; every in-tree caller uses a
            // TOTAL comparator (e.g. derived Ord over (n, word)) so observable
            // order is unchanged. Pure (! {}): swaps via ownership-neutral
            // vec_swap (non-Copy T ok), reads via vec_get_ref/vec_len — no alloc.
            // Recursion via the named helper `qsort` (a closure can't self-
            // recurse — see sort_by below); median-of-three bounds depth to
            // O(log n) on sorted/reverse inputs that kill a naive last pivot.
            "fn qsort<T: Ord>(v: &mut Vec<T>, lo: i64, hi: i64) ! {} {\n"
            "    if hi - lo > 12 {\n"
            "        let mid = lo + (hi - lo) / 2;\n"
            "        let last = hi - 1;\n"
            "        let c0 = vec_get_ref(v, mid).cmp(vec_get_ref(v, lo));\n"
            "        if c0 < 0 { vec_swap(v, lo, mid); } else {}\n"
            "        let c1 = vec_get_ref(v, last).cmp(vec_get_ref(v, lo));\n"
            "        if c1 < 0 { vec_swap(v, lo, last); } else {}\n"
            "        let c2 = vec_get_ref(v, mid).cmp(vec_get_ref(v, last));\n"
            "        if c2 < 0 { vec_swap(v, mid, last); } else {}\n"
            "        let mut store = lo;\n"
            "        let mut k = lo;\n"
            "        while k < last {\n"
            "            let ck = vec_get_ref(v, k).cmp(vec_get_ref(v, last));\n"
            "            if ck < 0 { vec_swap(v, k, store); store = store + 1; } else {}\n"
            "            k = k + 1;\n"
            "        }\n"
            "        vec_swap(v, store, last);\n"
            "        qsort(v, lo, store);\n"
            "        qsort(v, store + 1, hi);\n"
            "    } else {\n"
            "        let mut i = lo + 1;\n"
            ...
            "            while go {\n"
            "                if j > lo {\n"
            "                    let cj = vec_get_ref(v, j - 1).cmp(vec_get_ref(v, j));\n"
            "                    if cj > 0 { vec_swap(v, j - 1, j); j = j - 1; } else { go = false; }\n"
            "                } else { go = false; }\n"
            "            }\n"
            "            i = i + 1;\n"
            "        }\n"
            ...
            "    qsort(v, 0, n);\n"
            "}\n"
            // v103: sort_by(v, cmp) — quicksort with a CALLER comparator
            // `Fn(&T,&T)->i64` (neg/zero/pos, same convention as Ord::cmp).
            // ITERATIVE (explicit (lo,hi) stack) because a closure is a move-only
            // fat-ptr value: it cannot be threaded through a recursive helper (each
            // recursive call would move it). The closure lives in this one frame.
            "fn sort_by<T>(v: &mut Vec<T>, cmp: Fn(&T, &T) -> i64) ! { alloc } {\n"
            "    let n = vec_len(v);\n"
            "    let mut los: Vec<i64> = vec_new();\n"
            "    let mut his: Vec<i64> = vec_new();\n"
            "    vec_push(&mut los, 0); vec_push(&mut his, n);\n"
            "    let mut go_outer = true;\n"
            "    while go_outer {\n"
            "        let sz = vec_len(&los);\n"
            "        if sz == 0 { go_outer = false; } else {\n"
            "            let lo = vec_pop(&mut los);\n"
            "            let hi = vec_pop(&mut his);\n"
            "            if hi - lo > 12 {\n"
            "                let mid = lo + (hi - lo) / 2;\n"
            "                let last = hi - 1;\n"
            "                let c0 = cmp(vec_get_ref(v, mid), vec_get_ref(v, lo));\n"
            "                if c0 < 0 { vec_swap(v, lo, mid); } else {}\n"
            "                let c1 = cmp(vec_get_ref(v, last), vec_get_ref(v, lo));\n"
            "                if c1 < 0 { vec_swap(v, lo, last); } else {}\n"
            "                let c2 = cmp(vec_get_ref(v, mid), vec_get_ref(v, last));\n"
            "                if c2 < 0 { vec_swap(v, mid, last); } else {}\n"
            "                let mut store = lo;\n"
            "                let mut k = lo;\n"
            "                while k < last {\n"
            "                    let ck = cmp(vec_get_ref(v, k), vec_get_ref(v, last));\n"
            "                    if ck < 0 { vec_swap(v, k, store); store = store + 1; } else {}\n"
            "                    k = k + 1;\n"
            "                }\n"
            "                vec_swap(v, store, last);\n"
            "                vec_push(&mut los, lo); vec_push(&mut his, store);\n"
            "                vec_push(&mut los, store + 1); vec_push(&mut his, hi);\n"
            "            } else {\n"
            "                let mut i = lo + 1;\n"
            "                while i < hi {\n"
            "                    let mut j = i;\n"
            ...
            "                    while goj {\n"
            "                        if j > lo {\n"
            "                            let cj = cmp(vec_get_ref(v, j - 1), vec_get_ref(v, j));\n"
            "                            if cj > 0 { vec_swap(v, j - 1, j); j = j - 1; } else { goj = false; }\n"
            "                        } else { goj = false; }\n"
            "                    }\n"
            "                    i = i + 1;\n"
            "                }\n"
            ...
            "    }\n"
            "}\n"
            // binary_search_by(v, target, cmp): the vec MUST already be sorted by
            // the same comparator. Some(index of a match) / None. No alloc.
            "fn binary_search_by<T>(v: &Vec<T>, target: &T, cmp: Fn(&T, &T) -> i64) -> Option<i64> ! {} {\n"
            "    let mut lo = 0;\n"
            "    let mut hi = vec_len(v);\n"
            "    let mut res: Option<i64> = None;\n"
            "    let mut go = true;\n"
            "    while go {\n"
            "        if lo < hi {\n"
            "            let mid = lo + (hi - lo) / 2;\n"
            "            let c = cmp(vec_get_ref(v, mid), target);\n"
            "            if c < 0 { lo = mid + 1; }\n"
            "            else { if c > 0 { hi = mid; } else { res = Some(mid); go = false; } }\n"
            "        } else { go = false; }\n"
            "    }\n"
            "    res\n"
            "}\n"
            // binary_search(v, target): Ord-based, over a vec sorted by `sort`.
            "fn binary_search<T: Ord>(v: &Vec<T>, target: &T) -> Option<i64> ! {} {\n"
            "    binary_search_by(v, target, |a, b| a.cmp(b))\n"
            "}\n"
            // partition(v, pred): in-place — all elements satisfying `pred` moved
            // to the front; returns the count (the pivot index). Only vec_swaps,
            // so ! {} (effect-free). T unbounded.
            "fn partition<T>(v: &mut Vec<T>, pred: Fn(&T) -> bool) -> i64 ! {} {\n"
            "    let n = vec_len(v);\n"
            "    let mut store = 0;\n"
            "    let mut k = 0;\n"
            "    while k < n {\n"
            "        let keep = pred(vec_get_ref(v, k));\n"
            "        if keep { vec_swap(v, k, store); store = store + 1; } else {}\n"
            "        k = k + 1;\n"
            "    }\n"
            "    store\n"
            "}\n";
    ...
    if (userSrc.find("fn vec_contains") == std::string::npos) {
        prelude +=
            ...
            "    let mut found = false;\n"
            "    while i < vec_len(v) {\n"
            "        if vec_get_ref(v, i).eq(x) { found = true; break; } else {}\n"
            "        i = i + 1;\n"
            "    }\n"
            "    found\n"
            "}\n";
    ...
    if (userSrc.find("fn vec_index_of") == std::string::npos) {
        prelude +=
            ...
            "    let mut idx = 0 - 1;\n"
            "    while i < vec_len(v) {\n"
            "        if vec_get_ref(v, i).eq(x) { idx = i; break; } else {}\n"
            "        i = i + 1;\n"
            "    }\n"
            "    idx\n"
            "}\n";
    ...
    if (userSrc.find("fn vec_take") == std::string::npos) {
        prelude +=
            ...
            "    let mut i = 0;\n"
            "    let mut found = false;\n"
            "    while i < vec_len(v) { if pred(vec_get_ref(v, i)) { found = true; break; } else {} i = i + 1; }\n"
            "    found\n"
            "}\n"
            "fn vec_all<T, e>(v: &Vec<T>, pred: fn(&T) -> bool ! {e}) -> bool ! { alloc, e } {\n"
            "    let mut i = 0;\n"
            "    let mut ok = true;\n"
            "    while i < vec_len(v) { if pred(vec_get_ref(v, i)) {} else { ok = false; break; } i = i + 1; }\n"
            "    ok\n"
            "}\n"
            ...
            "    let mut idx = 0 - 1;\n"
            "    while i < vec_len(v) {\n"
            "        if pred(vec_get_ref(v, i)) { idx = i; break; } else {}\n"
            "        i = i + 1;\n"
            "    }\n"
